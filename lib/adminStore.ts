//lib/adminStore.ts
import { toISODate, weekdayIndexFromISO } from "@/lib/date";
...
function seed(): DB {
  const s1: Service = { id: uid(), name: "Uñas", description: "Servicio de uñas", durationMin: 30, active: true };
  const s2: Service = { id: uid(), name: "Criolipólisis", durationMin: 50, active: true };
  const s3: Service = { id: uid(), name: "Micropigmentación", durationMin: 50, active: true };

  const today = toISODate(new Date());

  const rules: ScheduleRule[] = [
    { id: uid(), serviceId: s3.id, weekday: 0, from: "09:00", to: "13:00" },
    { id: uid(), serviceId: s3.id, weekday: 2, from: "09:00", to: "13:00" },
  ];
...
export function computeAvailability(serviceId: string, dateISO: string) {
  ...
  const w = weekdayIndexFromISO(dateISO);
  const rules = db.rules.filter((r) => r.serviceId === serviceId && r.weekday === w);

  const allSlots = rules.flatMap((r) => generateSlots(r.from, r.to, service.durationMin, service.durationMin));
